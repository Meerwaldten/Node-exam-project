<script>
    import { BASE_URL, isUserAdmin, myUsername } from "../stores/globalStore";
    ...
    <div class="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
      <div class="relative flex h-16 items-center justify-between">
        ...
        <div class="flex flex-1 items-center justify-start">
          ...
          <div class="hidden sm:ml-6 sm:block">
            <div class="flex space-x-4">
              ...
              <Link to="/suggestion" class="text-gray-300 hover:bg-gray-700 hover:text-white rounded-md px-3 py-2 text-sm font-medium"> Suggestion </Link>
              {#if $myUsername}
              <Link to="/findcharacter" class="text-gray-300 hover:bg-gray-700 hover:text-white rounded-md px-3 py-2 text-sm font-medium"> Your characters </Link>
              {/if}
              {#if $isUserAdmin}
              <Link to="/admin" class="text-gray-300 hover:bg-gray-700 hover:text-white rounded-md px-3 py-2 text-sm font-medium"> Admin page </Link>
              {/if}
            </div>
          </div>
        </div>
        <div class="relative inline-block text-left right-0">
          ...
          type="button" on:click={handleDropdownClick}> 
          Profile 
